...
export class OrunmilangValidator {
    ...
    checkMultiplicativeExpression(expr, accept) {
        const checkOperandType = (operand) => {
            // Allow variables of unknown type (they'll be checked at runtime)
            if (operand.$type === 'VariableReference')
                return true;
    ...
    checkArrayLiteral(array, accept) {
    ...
    checkArrayAccess(access, accept) {
        if (!access.array) {
    ...
    checkArrayAssignment(assign, accept) {
        if (!assign.array) {
